...
################################################################################
# cv
count = count[(count['time_stamp']>=pd.datetime(2016,10,4)) & (count['time_stamp']<=pd.datetime(2016,10,31))]


#每个shop 一个样本
# # #train
# count = count[(count['time_stamp']>=pd.datetime(2016,6,28)) & (count['time_stamp']<=pd.datetime(2016,7,25))]
# #cv
# count = count[(count['time_stamp']>=pd.datetime(2016,7,5)) & (count['time_stamp']<=pd.datetime(2016,8,1))]
...
paycount_feature.to_csv('./cl2/cv_countcl_featureTG14.csv',index = False)
# paycount_feature.to_csv('./cl2/train7_countcl_featureTG14.csv',index = False)
