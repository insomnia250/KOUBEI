...
# ################################################################################
# # cv
# count = count[(count['time_stamp']>=pd.datetime(2016,10,11)) & (count['time_stamp']<=pd.datetime(2016,10,31))]
# #train
# count = count[(count['time_stamp']>=pd.datetime(2016,10,4)) & (count['time_stamp']<=pd.datetime(2016,10,24))]

#每个shop 多个样本
# #train
count = count[(count['time_stamp']>=pd.datetime(2016,7,12)) & (count['time_stamp']<=pd.datetime(2016,10,24))]
# #cv
# count = count[(count['time_stamp']>=pd.datetime(2016,10,11)) & (count['time_stamp']<=pd.datetime(2016,10,31))]
...
# #每个shop 多个样本
# # #train
# count = count[(count['time_stamp']>=pd.datetime(2016,8,16)) & (count['time_stamp']<=pd.datetime(2016,10,17))]
# #cv
# count = count[(count['time_stamp']>=pd.datetime(2016,10,4)) & (count['time_stamp']<=pd.datetime(2016,10,24))]
# #cv
# count = count[(count['time_stamp']>=pd.datetime(2016,10,11)) & (count['time_stamp']<=pd.datetime(2016,10,31))]

count = count[(count['time_stamp']>pd.datetime(2016,10,10)) | (count['time_stamp']<pd.datetime(2016,10,1))]
count = count[(count['time_stamp']>pd.datetime(2016,9,19)) | (count['time_stamp']<pd.datetime(2016,9,13))]
count = count[(count['time_stamp']>pd.datetime(2016,8,15)) | (count['time_stamp']<pd.datetime(2016,8,9))]
...
deltaday = pd.to_timedelta('7 days')  #滑动窗的间隔4天
for i,shop in enumerate(shoplist):
	...
	dayend = shopdata['time_stamp'].max() - pd.to_timedelta('20 days') #滑动窗的长度21天
	...
	for day in pd.date_range(start = daystart , end = dayend , freq=deltaday):
		...
		# print day 
		# print pd.date_range(start = daystart , end = dayend , freq=deltaday)
		# print ftday
		# print tgday
...
# paycount_feature.to_csv('./cl2/cv_countcl_featureTG7.csv',index = False)
paycount_feature.to_csv('./cl2/train_countcl_featureTG7.csv',index = False)
