...
count = pd.merge(count,shopinfo[['shop_id','city_name']],how='left',on='shop_id')
...
#每个shop 一个样本
# # #train
# count = count[(count['time_stamp']>=pd.datetime(2016,6,28)) & (count['time_stamp']<=pd.datetime(2016,7,18))]
# #cv
# count = count[(count['time_stamp']>=pd.datetime(2016,7,5)) & (count['time_stamp']<=pd.datetime(2016,7,25))]
# #cv
# count = count[(count['time_stamp']>=pd.datetime(2016,7,12)) & (count['time_stamp']<=pd.datetime(2016,8,1))]
