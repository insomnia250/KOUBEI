...
# training
score_multiXGB = []; score_avg = [];  score_opt = [];  score_train=[]
...
bst = XGBModel(max_depth=8, learning_rate=0.04, n_estimators=150, silent=1, 
		gamma=0.1, min_child_weight=0.3, subsample=0.6, colsample_bytree=0.3, seed=69)
...
for day in range(7):
	...
	score_multiXGB.append( calsore(predcv[:,day] , y_cv.ix[:,day]) )
...
result_cv.to_csv('multiXGB_cv.csv',index=False)
result_test.to_csv('multiXGB_test.csv',index=False,header=None)
...
print 'score_multiXGB:', score_multiXGB
