...
tgweather_train = df_train.ix[:,36:43]
cmft_train = df_train.ix[:,43:]
...
tgweather_test = df_test.ix[:,29:36]
cmft_test = df_test.ix[:,36:]
...
# training
score_ET = []; score_avg = [];  score_opt = [];  score_train=[]
...
for day in range(7):
	...
	ET
	'''
	clf_ET = ExtraTreesRegressor(n_estimators=50, criterion='mse', max_depth=7 , min_samples_split=2, 
	...
	# gsearch1 = GridSearchCV(estimator = clf_ET, param_grid = parameters_for_testing,iid=False,verbose=True)
	# gsearch1.fit(X,y)
	...
	# with open('./ET_models/clf_ET.pkl','w') as f:cPickle.dump(clf_ET,f)
	predtrain = clf_ET.predict(X)
	#cv
	predcv = clf_ET.predict(X_cv)
	result_cv['day_'+str(day)] = list(predcv)

	score_train.append( calsore(predtrain , y) )
	score_ET.append( calsore(predcv , y_cv) )
	...
	print X_cv.info()
	print X_test.info()
	predtest = clf_ET.predict( X_test) 
	result_test['day_'+str(day)] = list(predtest)



result_cv.to_csv('ET_cv.csv',index=False)
result_test.to_csv('ET_test.csv',index=False,header=None)
print 'score_train:', np.mean(score_train)
print 'score_ET:', np.mean(score_ET)
