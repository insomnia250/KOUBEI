...
for i,shop in enumerate(uniqueshop):
	...
	target = shopdata[(shopdata['time_stamp']>=pd.datetime(2016,7,17)) & (shopdata['time_stamp']<=pd.datetime(2016,7,30))]

	avg1 = fourteen['count'].mean()

	oct1 = shopdata.loc[(shopdata['time_stamp']>=pd.datetime(2016,7,3)) & (shopdata['time_stamp']<=pd.datetime(2016,7,9))]
	oct2 = shopdata.loc[(shopdata['time_stamp']>=pd.datetime(2016,7,10)) & (shopdata['time_stamp']<=pd.datetime(2016,7,16))]
	...
	avgpre = np.array([avg1 * w6,avg1 * w0,avg1 * w1,avg1 * w2,avg1 * w3,avg1 * w4,avg1 * w5,
		avg1 * w6,avg1 * w0,avg1 * w1,avg1 * w2,avg1 * w3,avg1 * w4,avg1 * w5])
	xgbpre = XGB.loc[XGB['shop_id']==shop].values[0,1:]

	xgbavg = xgbpre.mean()
	xgb_fw = np.array([xgbavg * w6,xgbavg * w0,xgbavg * w1,xgbavg * w2,xgbavg * w3,xgbavg * w4,xgbavg * w5,
		xgbavg * w6,xgbavg * w0,xgbavg * w1,xgbavg * w2,xgbavg * w3,xgbavg * w4,xgbavg * w5])

	y = list(target['count'])

	score_avg = (np.abs( (avgpre - y)/ (avgpre + y))).sum()/14.0
	score_xgb = (np.abs( (xgbpre - y)/ (xgbpre + y))).sum()/14.0
	score_xgbfw = (np.abs( (xgb_fw - y)/ (xgb_fw + y))).sum()/14.0


	if (score_xgb - score_avg)/score_avg>-0.1:
		c+=1
		xgbdamnshop['shop_id'].append(shop)
		xgbdamnshop['error'].append((score_xgb - score_avg)/score_avg)
