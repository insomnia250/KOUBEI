...
xgbdamnshop = pd.read_csv('xgbdamnshop.csv')
xgbdamnshop = list(xgbdamnshop['shop_id'])

xgbdamnshop.extend([489,1856,224,1366,562,674,1737,1218,530,352,561,1561])
print XGBresult.info()

for i,idx in enumerate( XGBresult.index):
	if XGBresult.loc[idx]['shop_id'] in xgbdamnshop:
		XGBresult.drop(idx,axis=0,inplace=True)
print XGBresult.info()
...
result.to_csv('XGB{0}+avg1+w365-oct_sigma.csv'.format(len(XGBresult)),index=False,header=None,encoding = 'utf-8')
